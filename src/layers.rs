...
impl Plugin for LayersPlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<SimpleStore<Layer>>()
            ...
            .add_systems(Update, update_active_clips);
    }
}
...
fn update_active_clips(
    ...
    mut effects_query: Query<&mut Effect>,
) {
    ...
    layer_queue.push_back((primary_layer_handle, playback_information.current_time));
                ...
                ClipInstanceRef::Effect {
                    ...
                    if is_in_clip {
                        if let Some(some_entity) = entity {
                            if let Ok(mut effect_component) = effects_query.get_mut(*some_entity) {
                                effect_component.current_time = new_clip_time;
                            } else {
