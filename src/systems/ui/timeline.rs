...
pub fn draw_timeline(
    ...
    let (response, painter) =
        ui.allocate_painter(Vec2::new(ui.available_width(), 50.0), Sense::click_and_drag());
    ...
    if response.clicked() || response.dragged() {
        ...
        if let Some(pos) = click_pos {
            ...
            playback.current_time = playback.length * percentage as f64;
        }
    }

    let playhead_pos = response.rect.min + Vec2::new(
        (playback.current_time / playback.length) as f32 * response.rect.width(),
        0.0,
    );
    ...
    // bar lines
    let num_bars = (playback.length * playback.bpm / 60.0).ceil() as usize;
    for i in 0..num_bars {
    ...
    for pos in keyframe_positions {
        let x = pos as f32 / playback.length as f32 * response.rect.width();
